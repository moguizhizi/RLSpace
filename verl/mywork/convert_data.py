...
MODEL_PATH = "/data/sk/project/drug_recommendation/Models/Qwen3-8B-SFT-Merged-TFG/0121" 
...
def make_dataset(json_path, output_parquet_path):
    ...
    for example in ds:
        ...
        data_list.append({
            ...
            "prompt": [{"role": "user", "content": prompt}], # VeRL 接收 chat list 或 string，这里存 raw string 更好处理
            ...
            "extra_info": {
                "original_input": example['input'] # 需要保留原始 input 来提取 candidates
            }
        })
...
if __name__ == "__main__":
    d1 = make_dataset("/data/sk/project/drug_recommendation/MR/data_think/output/TFG1/test_sft.json", "mywork/data/test.parquet")
    d2 = make_dataset("/data/sk/project/drug_recommendation/MR/data_think/output/TFG1/train_sft.json", "mywork/data/train.parquet")
    d3 = make_dataset("/data/sk/project/drug_recommendation/MR/data_think/output/TFG1/val_sft.json", "mywork/data/val.parquet")
    print(len(d1), len(d2), len(d3))
    d_val = d3[:100]
    d_train = pd.concat([d2, d3[100:]], ignore_index=True)
    d_test = d1
    print(len(d_train), len(d_val), len(d_test))
    d_train.to_parquet("mywork/data/train.parquet")
    d_val.to_parquet("mywork/data/val.parquet")
    d_test.to_parquet("mywork/data/test.parquet")
    print("Done!")